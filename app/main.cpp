...
struct algorithm
{
    enum code {wise = 0, clas, entropy, hamming};
    algorithm(const std::string &n): name{n}{}
    code toCode() const
    {
	if(name=="wise") return code::wise;
	if(name=="class") return code::clas;
	if(name=="entropy") return code::entropy;
	if(name=="hamming") return code::hamming;
...
int main(int argc, char const *argv[])
{
	...
	desc.add_options()
	    ...
	    ("algorithm,a", po::value<algorithm>(), 
             "Aditum algorithm\n"
	     "Options:\n"
	     "    [*] wise - attribute wise\n"
	     "    [*] class - class based\n"
	...
	commonArgs.add_options()
	    ("graph", po::value<std::string>(), "input graph")
	    ("capital", po::value<std::string>(), "capital score")
	    ("target-threshold", po::value<double>(), "\u03C4")
	    ("output,out", po::value<std::string>(), "where seeds have to be stored")
	    ("diffusion-model,dm", po::value<diffusion>(),
	     "Diffusion model according to which the RR Sets must be generated.\n"
	     "Options:\n"
	     "    [*] lt - linear threshold model\n"
	     "    [*] ic - independent cascade model"
	     )
	    ("epsilon, eps", po::value<double>()->default_value(1), "\u03B5")
	    ("accuracy, l", po::value<double>()->default_value(1), "accuracy - l")
	    ("alpha, a", po::value<double>()->default_value(1), "\u03B1")
	    ("k", po::value<int>(), "number of seeds to be discovered");


	//attribute driven arguments
	po::options_description attrArgs("Attribute Arguments");
	attrArgs.add_options()
	    ("attributes", po::value<std::string>(), "file containing users attributes")
	    ("regex", po::value<std::string>()->default_value("\\s+"), "regex to be used when reading the user attributes file");

	//attribute wise diversity arguments
	po::options_description attrWiseArgs("AttributeWise Arguments");
	attrWiseArgs.add_options()
	    ("lambda, lam", po::value<double>()->default_value(1.0), "\u03BB");
	...
	hammingBased.add_options()
	    ("radius, r", po::value<std::string>()->default_value(""),"\u03BE")
	    ("reach", po::value<int>()->default_value(0), "Influence Reach Strategy.\n"
	     "Options:\n"
	     "    [0] - RRSet,  based on the computed RRSets\n"
	     "    [1] - Reach, based on the graph topology\n");

	desc.add(commonArgs).add(attrArgs).add(attrWiseArgs)
	    .add(classBasedArgs).add(hammingBased);
